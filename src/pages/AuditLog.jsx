...
import { AlertTriangle, Trash2, FileText, DollarSign, Briefcase, RefreshCw } from "lucide-react";
...
export default function AuditLog() {
  ...
  async function load() {
    ...
    // Flag jobs that reference an estimate or invoice that no longer exists
    const orphans = jobs.filter(job => {
      ...
      // Also flag jobs that HAVE no estimate AND no invoice at all (never linked)
  ...
  const deletedJobs = deletionLogs.filter(l => l.entity_type === "Job");
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Track deletions and surface jobs with missing financial records.</p>
        </div>
        ...
        <Tabs defaultValue="suspicious">
          <TabsList>
            <TabsTrigger value="suspicious">
              🚨 Suspicious Jobs ({suspiciousJobs.length})
            </TabsTrigger>
            <TabsTrigger value="deletions">
              🗑️ Deletion Log ({deletionLogs.length})
            </TabsTrigger>
            <TabsTrigger value="unlinked">
              🔗 Never Linked ({orphanedJobs.filter(j => j.neverLinked).length})
            </TabsTrigger>
          </TabsList>

          {/* Suspicious jobs — had a reference that's now gone */}
                  ...
                  <div className="space-y-3">
                    {suspiciousJobs.map(job => (
                      <div key={job.id} className="border border-red-100 rounded-lg p-4 bg-red-50">
                        ...
                        {/* Check deletion log for matching records */}
                        {(() => {
            ...
            <Card>
              ...
              <CardContent>
                {deletionLogs.length === 0 ? (
                  <p className="text-slate-400 text-sm">No deletions logged yet. Audit logging is now active going forward.</p>
                ) : (
